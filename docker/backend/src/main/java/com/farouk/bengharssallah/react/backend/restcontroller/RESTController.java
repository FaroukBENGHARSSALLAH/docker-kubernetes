...
public class RESTController {
	...
	@GetMapping(value = "/api/etfs/{country}/{page}")
	...
	@GetMapping(value = "/api/etf/{ticker}")
	...
	@GetMapping(value = "/api/pop")
	public ResponseEntity<String> country() throws Exception {
		System.out.println("starting copying DB ****************************");
		ObjectMapper objectMapper = new ObjectMapper();
		List<ETF> etfs = objectMapper.readValue(ResourceUtils.getFile( "classpath:etfs.json"), 
				new TypeReference<List<ETF>>(){});
		etfs.stream().forEach(e -> etfService.add(e));
		System.out.println("ending copying DB" + etfs.size() + " ****************************");
